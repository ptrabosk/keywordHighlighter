...
The background service worker owns the local event queue, retry policy, and HTTPS uploads. The logging sanitizer is the client-side data boundary: shortcut events can contain only a normalized shortcut and a bounded highlight count. Google Apps Script is the server-side boundary. It authenticates with a rotatable shared ingestion token, validates the same event contract, applies daily quotas under a script lock, writes accepted rows to Google Sheets, and deletes shortcut rows plus their deduplication IDs after 90 days.
